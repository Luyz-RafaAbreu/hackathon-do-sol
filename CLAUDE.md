...
- A planilha tem **183 colunas** numa linha por equipe: meta (6) + equipe (8) + proposta (4) + aceites coletivos (1 concat) + integrante (41 colunas × 4). `setup()` cria tudo via `COLUMNS` array — se mudar campos no schema, atualizar tanto `lib/inscricao-schema.ts` quanto o `FIELD_MAX` no topo do `apps-script.gs`.
- E-mails de confirmação, aprovação e reprovação vão **só pro líder**, no endereço da conta Google que ele usou pra logar no site (`leaderGoogleEmail`). Os e-mails pessoais dos 4 integrantes ficam só pra registro na planilha.
